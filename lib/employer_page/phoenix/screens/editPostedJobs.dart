...
class EditPostJob extends StatefulWidget {
  final MyPosted job;
  EditPostJob(this.job);

  @override
  State<EditPostJob> createState() => _EditPostJobState();
}

class _EditPostJobState extends State<EditPostJob> {
  String jobType = '';
  ...
  @override
  void initState() {
    jobController.text = widget.job.title!;
    jobDescController.text = widget.job.description!;
    categoryPost = widget.job.categories!;
    jobTypePost = widget.job.jobType!;
    salary_type = widget.job.salary_type!;
    stringStart = DateFormat('yyyy-MM-dd').format(widget.job.expiry);
    currency = widget.job.currency!;
    budgetController.text = widget.job.budget!;
    allowance.text = widget.job.benefit!;
    jobReqirement.text = widget.job.requirement!;
    jobQualification.text = widget.job.qualification!;
    isRemote = widget.job.isRemote!;
    cityValue = widget.job.location!.split(',')[0];
    stateValue = widget.job.location!.split(',')[1];
    countryValue = widget.job.location!.split(',')[2];
    super.initState();
  }
  ...
  Widget step1() => SingleChildScrollView(
        child: Column(
          ...
          children: [
            ...
            buildPaddingDropdown(jobCategoryItem.toList(), '${categoryPost}',
                callBack: (s) => {
                      setState(() {
                        categoryPost = s;
                      })
            ...
            Container(
              ...
              child: DateTimePicker(
                ...
                dateMask: 'yyyy-MM-dd',
                initialValue:
                    DateFormat('yyyy-MM-dd').format(widget.job.expiry),
  ...
  executeData() async {
    ...
    try {
      final res = await http.Client().post(
          Uri.parse('${ROOT}job_update/${widget.job.jobKey}'),
          body: data,
          headers: {
      ...
      CustomSnack('Error', 'Can not submit request..');
    } finally {
      setState(() {
        isLoading = false;
      });
