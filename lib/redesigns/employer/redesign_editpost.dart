...
class ReEditPostedJob extends StatefulWidget {
  final MyPosted jobs;
  const ReEditPostedJob(this.jobs, {super.key});

  @override
  State<ReEditPostedJob> createState() => _ReEditPostedJobState();
}

class _ReEditPostedJobState extends State<ReEditPostedJob> {
  ...
  void initState() {
    fetchData(context).then((value) {
      setState(() {
        compModel = value;
        benefits = widget.jobs.benefit!.split(', ');
        skills = widget.jobs.benefit!.split(', ');
        jobQualification.text = widget.jobs.qualification!;
        jobDescController.text = widget.jobs.description!;
        jobReqirement.text = widget.jobs.requirement!;
        jobController.text = widget.jobs.title!;
        categories = widget.jobs.categories!;
  ...
  executeData() async {
    ...
    try {
      final res = await http.Client().post(
          Uri.parse('${ROOT}job_update/${widget.jobs.jobKey}'),
          body: data,
          headers: {
